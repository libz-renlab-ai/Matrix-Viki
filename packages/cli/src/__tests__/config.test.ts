...
describe("writeVikiConfig", () => {
  let tmp: ReturnType<typeof mkTmp>;
  beforeEach(() => { tmp = mkTmp(); });
  afterEach(() => { tmp.cleanup(); });

  it("creates .viki dir and writes config", () => {
    writeVikiConfig(tmp.cwd, { stop_mode: "async" });
    const raw = fs.readFileSync(path.join(tmp.cwd, ".viki", "config.json"), "utf-8");
  ...
  it("merges into existing config", () => {
    ...
    writeVikiConfig(tmp.cwd, { stop_mode: "sync" });
...
describe("readVikiConfig walk-up (#161)", () => {
  let tmp: ReturnType<typeof mkTmp>;
  beforeEach(() => { tmp = mkTmp(); });
  afterEach(() => { tmp.cleanup(); });
  ...
  it("reads config from parent when cwd is a subfolder", () => {
    ...
    fs.writeFileSync(path.join(dir, ".viki", "knowledge.db"), "");
    fs.writeFileSync(path.join(dir, ".viki", "config.json"), JSON.stringify({ stop_mode: "sync" }));
    fs.writeFileSync(path.join(dir, "package.json"), "{}");
    // Read from subfolder
    const sub = path.join(dir, "sub");
    fs.mkdirSync(sub, { recursive: true });
    const cfg = readVikiConfig(sub);
