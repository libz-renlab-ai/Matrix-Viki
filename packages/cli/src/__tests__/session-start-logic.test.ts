...
describe("decideAction", () => {
  ...
  it("无 .viki/knowledge.db + 非项目目录 → skip-not-a-project", () => {
  ...
  it("无 db + .viki/auto-init.disabled 存在 → skip-auto-init-disabled", () => {
    writeFileSync(join(cwd, "package.json"), "{}");
    mkdirSync(join(cwd, ".viki"), { recursive: true });
    writeFileSync(join(cwd, ".viki", "auto-init.disabled"), "");
  ...
  it("已存在 knowledge.db → skip-already-initialized", () => {
    ...
    mkdirSync(join(cwd, ".viki"), { recursive: true });
    writeFileSync(join(cwd, ".viki", "knowledge.db"), "");
    mkdirSync(join(cwd, ".git"), { recursive: true });
  ...
  // .viki/knowledge.db so SessionStart from a sub-directory does not
  ...
  describe("issue #161 ancestor-aware walk-up", () => {
    ...
    beforeEach(() => {
      root = mkdtempSync(join(tmpdir(), "viki-w4-"));
      sub = join(root, "sub");
    ...
    it("ancestor 有 .viki/knowledge.db + project-marker，子目录调用 → skip-already-initialized", () => {
      // PR #181: walk-up requires both knowledge.db AND project-marker.
      mkdirSync(join(root, ".viki"), { recursive: true });
      writeFileSync(join(root, ".viki", "knowledge.db"), "");
      mkdirSync(join(root, ".git"), { recursive: true });
      // sub itself has no .viki/knowledge.db
      expect(decideAction(sub, new Date())).toBe("skip-already-initialized");
    });

    it("无任何 ancestor 有 db + cwd 是项目目录 → auto-init", () => {
      // No .viki anywhere along the chain; cwd has package.json marker.
      writeFileSync(join(sub, "package.json"), "{}");
      expect(decideAction(sub, new Date())).toBe("auto-init");
    });

    it("无 ancestor db + cwd 有 .viki/auto-init.disabled → skip-auto-init-disabled", () => {
      // No knowledge.db anywhere; only the per-cwd disabled marker exists.
      mkdirSync(join(sub, ".viki"), { recursive: true });
      writeFileSync(join(sub, ".viki", "auto-init.disabled"), "");
    ...
    // child cwd. Scenario: ancestor has .git + .viki/auto-init.disabled
    ...
    it("PR #181: ancestor's auto-init.disabled marker is honored from a child cwd (no ancestor knowledge.db)", () => {
      // Plant ancestor: .git + .viki/auto-init.disabled, NO knowledge.db.
      mkdirSync(join(root, ".git"), { recursive: true });
      mkdirSync(join(root, ".viki"), { recursive: true });
      writeFileSync(join(root, ".viki", "auto-init.disabled"), "");
...
describe("maybeShowReinstallBanner (B-104)", () => {
  let vikiDir: string;          // 直接对应 ~/.viki
  let originalEnv: string | undefined;

  beforeEach(() => {
    // session-start-logic 的 vikiHome() 把 VIKI_HOME 直接当作
    // .viki 目录用（不再追加 .viki 子目录）。所以测试必须把 env
    // 指向最终的 .viki 目录本身。
    const root = mkdtempSync(join(tmpdir(), "rb-"));
    vikiDir = join(root, ".viki");
    mkdirSync(vikiDir, { recursive: true });
    originalEnv = process.env.VIKI_HOME;
    process.env.VIKI_HOME = vikiDir;
  });
  afterEach(() => {
    if (originalEnv === undefined) delete process.env.VIKI_HOME;
    else process.env.VIKI_HOME = originalEnv;
    rmSync(vikiDir, { recursive: true, force: true });
  });

  function writeState(state: object): void {
    writeFileSync(
      join(vikiDir, "update-state.json"),
  ...
  it("consecutive_install_failures>=1 + 有 error → 显示红字 + 重装命令", () => {
    ...
    const after = JSON.parse(
      readFileSync(join(vikiDir, "update-state.json"), "utf-8"),
    );
...
describe("maybeShowVersionCheckBanner (issue #313 Tier 3)", () => {
  ...
  it("last_install_error 带 'version-check failed:' 前缀 → 显示 Tier 3 banner + 3 条恢复路径", () => {
    ...
    // 三条恢复路径必须都出现
    expect(captured).toContain("npm i -g viki@latest");
    expect(captured).toContain("等下次启动");
    expect(captured).toContain("VIKI_GITHUB_TOKEN");
