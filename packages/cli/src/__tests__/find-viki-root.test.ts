/**
 * Tests for findVikiRoot wrapper (cli/src/find-viki-root.ts).
 *
 * The wrapper delegates to `lib/walk-up.ts#findVikiRoot` (hardened) and
 * supplies a `?? cwd` fallback so callers that need a `string` keep working.
 *
 * Hardened helper requires BOTH:
 *   (a) `<dir>/.viki/knowledge.db` is a regular file
 ...
 *
 * Issue #161 — subfolder cwd can't load viki: hook entrypoints previously
 * did `path.join(cwd, ".viki", "knowledge.db")` without ancestor walk-up.
 */
...
function makeTmpDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "viki-root-test-"));
  tmpdirs.push(dir);
  return dir;
}
...
/** Create `.viki/knowledge.db` (empty file) AND a project marker under `dir`. */
function createProject(dir: string): void {
  const dbDir = path.join(dir, ".viki");
...
describe("findVikiRoot wrapper", () => {
  it("returns cwd when cwd itself has .viki/knowledge.db + project marker", () => {
    const root = makeTmpDir();
    createProject(root);
    expect(path.resolve(findVikiRoot(root))).toBe(path.resolve(root));
  });
  ...
  it("returns parent when cwd is a subdirectory and parent is a project", () => {
    ...
    expect(path.resolve(findVikiRoot(sub))).toBe(path.resolve(root));
  });
  ...
  it("returns grandparent when cwd is two levels deep and grandparent is a project", () => {
    ...
    expect(path.resolve(findVikiRoot(grandchild))).toBe(path.resolve(root));
  });

  it("returns cwd unchanged when no ancestor has the DB", () => {
    const root = makeTmpDir();
    const sub = path.join(root, "no-viki", "sub");
    fs.mkdirSync(sub, { recursive: true });
    expect(path.resolve(findVikiRoot(sub))).toBe(path.resolve(sub));
  });
  ...
  it("terminates at filesystem root and returns cwd (no infinite loop)", () => {
    ...
    expect(path.resolve(findVikiRoot(deep))).toBe(path.resolve(deep));
  });

  it("normalizes paths (trailing separator)", () => {
    const root = makeTmpDir();
    createProject(root);
    expect(path.resolve(findVikiRoot(root + path.sep))).toBe(path.resolve(root));
  });
  ...
  it("returns nearest ancestor when both grandparent and parent are projects", () => {
    ...
    fs.mkdirSync(sub, { recursive: true });
    expect(path.resolve(findVikiRoot(sub))).toBe(path.resolve(mid));
  });
  ...
  it("rejects a symlink at <dir>/.viki/knowledge.db (lstat hardening)", () => {
    const root = makeTmpDir();
