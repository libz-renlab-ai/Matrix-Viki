 ...
 *   - findVikiRoot (walk-up.ts) — security boundary; only directories
 ...
 *
 * The list is intentionally generic: a docs-only Viki project that
 * lacks any of these markers also gets a Viki-managed
 * `.viki/.project-root` marker on init, which is included below so
...
export const PROJECT_MARKERS = [
  ...
  "Gemfile",
  "composer.json",
  // Viki-managed marker. `viki init` writes
  // `<dir>/.viki/.project-root` on first run so docs-only projects
