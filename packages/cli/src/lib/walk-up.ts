 ...
 *  (a) `<dir>/.viki/knowledge.db` exists as a regular file (lstat — does
 ...
 *      Viki-managed `.viki/.project-root` marker written by
 *      `viki init` so docs-only projects are still discoverable).
 *
 ...
 * attacker-planted `/tmp/.viki/knowledge.db` (and similar locations
 ...
 * stray `.viki/knowledge.db` upstream of `cwd` would otherwise hijack
 ...
 *   `viki walk-up: <code> at <candidate>; aborting walk\n`
 ...
 * `<dir>/.viki/knowledge.db` pointing at a foreign file is rejected.
 * If a real workflow needs to alias one project's DB into another repo, the
 * recommendation is to add a separate marker (e.g. `.viki-link`)
 ...
 * find the project's `.viki/knowledge.db` registered in an ancestor.
 *
 * Behaviour notes:
 * - "First match wins, starting at cwd inclusive" — if both `cwd` and an
 *   ancestor have a `.viki/knowledge.db` (and a project marker),
 *   `cwd` wins.
 * - If `<dir>/.viki/knowledge.db` exists but is NOT a regular file
...
export function findVikiRoot(
  ...
  while (true) {
    const candidate = path.join(cur, ".viki", "knowledge.db");
    try {
      ...
      if (code === "ENOENT" || code === "ENOTDIR" || code === "EISDIR") {
        ...
        process.stderr.write(
          `viki walk-up: ${code ?? String(err)} at ${candidate}; aborting walk\n`,
        );
        return null;
      }
    }
