...
#   1. 在临时项目里跑 viki m5-infect
#   2. 验证 manifest + 共享层骨架已就位
#   3. 把项目 git init + commit + 模拟 push（local bare repo）
#   4. 在另一个临时目录 clone
#   5. 跑 viki m5-bootstrap，确认它读到了 manifest 并报告 diff
#
# 跑法（在 worktree 根目录）：
...
# Step 1: infect
( cd "$REPO_ROOT" && pnpm viki m5-infect --project-root "$ALICE" --author alice --viki-version 0.9.4 )

# Step 2: 验证 artifacts
test -f "$ALICE/.viki/manifest.json"   || { echo "missing manifest"; exit 1; }
test -d "$ALICE/.viki/team"             || { echo "missing team dir"; exit 1; }
test -d "$ALICE/.viki/shared-skills"    || { echo "missing shared-skills dir"; exit 1; }
test -f "$ALICE/.viki/shared-claude.md" || { echo "missing shared-claude.md"; exit 1; }
test -f "$ALICE/.githooks/pre-commit"        || { echo "missing pre-commit hook"; exit 1; }
echo "[m5-demo] infect OK"

# 验证 manifest 内容
grep -q '"created_by": "alice"'       "$ALICE/.viki/manifest.json" || { echo "manifest missing created_by=alice"; exit 1; }
grep -q '"viki_version": "0.9.4"' "$ALICE/.viki/manifest.json" || { echo "manifest missing viki_version=0.9.4"; exit 1; }
echo "[m5-demo] manifest content OK"

# Step 3: commit + bare-repo "push"
( cd "$ALICE" && git add . && git commit -qm "feat: infect project with Viki contract" )
...
test -f "$BOB/.viki/manifest.json" || { echo "clone missing manifest"; exit 1; }
echo "[m5-demo] clone OK, manifest present"

# Step 5: bootstrap reports diff (exit 2 表示需要补齐，是正常情况)
EXIT=0
( cd "$REPO_ROOT" && pnpm viki m5-bootstrap --project-root "$BOB" --check ) || EXIT=$?
