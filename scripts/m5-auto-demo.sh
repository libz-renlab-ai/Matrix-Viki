...
mkdir -p "$ALICE_HOME/.viki" "$BOB_HOME/.viki"
# Alice 已"装了 Viki"——伪装一个 global.db 让 userHasViki() 返回 true
touch "$ALICE_HOME/.viki/global.db"
...
echo "{\"cwd\":\"$ALICE_W\"}" | ( cd "$REPO" && HOME="$ALICE_HOME_W" VIKI_M5_AUTOSESSION=1 node --import tsx packages/cli/src/bin-session-start.ts 2>&1 | grep -v "Experimental" | head -20 )

echo
echo "===== Alice 项目应已被自动 infect ====="
ls "$ALICE/.viki/" 2>&1 || true
ls "$ALICE/.githooks/" 2>&1 || true

# ============ Step 2: Alice 通过 pitfall 加规则 → 自动 m5-share ============
echo
echo "===== Step 2: Alice pitfall 一条干净规则 → 应自动 promote 到 .viki/team/ ====="
...
echo
echo "===== .viki/team/alice/ 应有自动 promote 的文件 ====="
ls "$ALICE/.viki/team/" 2>&1 || true
ls "$ALICE/.viki/team/alice/" 2>&1 || true

# ============ Step 3: Alice m5-publish 自动 commit ============
echo
echo "===== Step 3: Alice m5-publish 自动 commit team 文件（demo 阶段没 origin，--no-push 跳过 push）====="
( cd "$ALICE" && pnpm --silent --dir "$REPO" viki m5-publish --project-root "$ALICE" --no-push 2>&1 | grep -v "Experimental" )
...
echo "{\"cwd\":\"$BOB_W\"}" | ( cd "$REPO" && HOME="$BOB_HOME_W" VIKI_M5_AUTOSESSION=1 node --import tsx packages/cli/src/bin-session-start.ts 2>&1 | grep -v "Experimental" | head -20 )

echo
echo "===== Bob 本地 KB 应已收到 Alice 的规则 ====="
test -f "$BOB/.viki/knowledge.db" && echo "knowledge.db 存在 ($(wc -c < "$BOB/.viki/knowledge.db") bytes)" || echo "knowledge.db NOT created"

echo
echo "===== Bob m5-status 应显示团队规则统计 ====="
pnpm --silent --dir "$REPO" viki m5-status --project-root "$BOB" 2>&1 | grep -v "Experimental"

echo
echo "===== Bob 端 m5-sync 显示当前 LWW 状态 ====="
pnpm --silent --dir "$REPO" viki m5-sync --project-root "$BOB" 2>&1 | grep -v "Experimental"

# ============ Step 7: Bob 的本地 KB 真的有 Alice 的规则吗？查 sqlite ============
echo
echo "===== Step 7: 直接查 Bob 的 knowledge.db 验证 Alice 的规则真的进 KB 了 ====="
if test -f "$BOB/.viki/knowledge.db"; then
  BOB_DB_W=$(cygpath -w "$BOB/.viki/knowledge.db" 2>/dev/null || echo "$BOB/.viki/knowledge.db")
  python -c "
import sqlite3
