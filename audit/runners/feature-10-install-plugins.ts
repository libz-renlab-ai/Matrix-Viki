...
const SCENARIOS: ScenarioSpec[] = [
  ...
  {
    name: "only-code-review-local-scope",
    label: "only code-review with local scope",
    args: ["--only=code-review", "--scope=local"],
    expectedExit: 0,
    expectedArgv: [
      ["plugin", "marketplace", "add", "anthropics/claude-plugins-official"],
      ["plugin", "install", "code-review@claude-plugins-official", "--scope", "local"],
    ],
    stdoutIncludes: ["code-review@claude-plugins-official", "2 新装"],
    stdoutExcludes: [
...
try {
  ...
  finalize(ctx, {
    ...
    summary:
      status === "passed"
        ? "通过：runner 创建临时 PATH fake claude，真实执行源码 CLI install-plugins 的默认、dry-run、only=code-review 和 unknown plugin 场景，并由独立 JSONL validator 校验 marketplace/install argv、HOME/PATH 隔离、scope/only/dry-run 行为和渲染输出。"
