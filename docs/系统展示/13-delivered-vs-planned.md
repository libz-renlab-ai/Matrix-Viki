...
| **Phase 4** — 团队层真正落地 | git-synced 知识共享、隐私脱敏、审核门 | ❌ **未实现**（本地 `scope=team` 已可写读；跨机器共享仍未完成）| v0.7.0 |
| **Phase 5** — 前沿 + 主动补充深化 | RAG 前沿、taste、全语义 Matcher v3 | ❌ 规划中 | v0.9.0 |
| **Phase 6** — 多 AI 工具 (MCP) | Cursor / Windsurf / ... 通过 MCP 接入 | ❌ 规划中 | v1.0.0（公开发布）|

**关键诚实披露**：
- **团队共享还没跑通**。本地 `scope=team` 已可写读和统计，但 Phase 4 的跨机器共享还没完成。向用户介绍时请不要把多人自动同步当已完成；`teamagent doctor --json` 会把 `team-sharing` 明确标为 `PARTIAL`，直到 git transport、privacy redaction、review gates 都存在。
