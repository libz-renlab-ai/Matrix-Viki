    ...
    <section>
      <div className="container">
        ...
        <h2 data-reveal style={{marginTop: 12}}>
          Crack it open.<br/>
          <span className="accent">Every rule, every fire, on your machine.</span>
        </h2>
        <p className="sub" data-reveal style={{marginTop: 24}}>
          Below: a real PreToolUse intercept, the real rule JSON schema, real SQLite queries.
          Switch tabs to see different angles — all reproducible after install via <span className="mono accent">viki demo / viki stats / sqlite3</span>.
        </p>
        ...
        <div data-reveal style={{
          ...
          {[
            { id: 'intercept', label: 'Real intercept output', sub: 'PreToolUse · stdout' },
            { id: 'rule', label: 'One rule, full schema', sub: 'rule#a3f7.json · 8 fields' },
            { id: 'stats', label: 'viki stats', sub: 'CLI output · grouped by tier' },
            { id: 'db', label: 'SQLite queries', sub: 'knowledge.db · raw' },
          ].map(t => (
...
{`> User: add me a date-formatting dep
...
<>
{`🚫 `}<span style={{color: 'var(--red)'}}>{`Viki intercept (confidence 0.85)`}</span>
{`
   Use instead: dayjs (API-compatible, ~2KB) or date-fns (tree-shakable)
   Why: moment.js has been in maintenance mode since 2020;
        large bundle + mutable API → subtle bugs
   (rule id: seed-pack-universal-moment)
`}
{`
...
⏺ PostToolUse → user accepted · rule confidence 0.85 → 0.91`}
      ...
      <div style={{gridColumn: '1 / -1', padding: 14, background: 'var(--bg-card)',
        ...
        ▸ Verbatim from README §6.1 + standard Claude Code session output format.
        Every line above can be reproduced after install via <span style={{color: 'var(--green)'}}>viki demo hook Bash 'command=npm install moment'</span>.
      </div>
      <style>{`
...
<>{`{
  ...
  "bypassed_count": 1
      ...
      <div style={{
        ...
        ▸ Schema defined in <span style={{color: 'var(--green)'}}>packages/types/src/rule.ts</span>.
        The first 8 fields are LLM-extracted from the correction moment; the rest (confidence/tier/counters) are maintained by the calibrator.
      </div>
    </div>
  );
}
...
`}</>
      ...
      <div style={{
        ...
        ▸ <span style={{color: 'var(--amber)'}}>Note</span>: this snapshot is from one developer's week of real use.
        The command itself runs on any installed machine — your numbers will reflect your actual activity.
      </div>
    </div>
  );
}
